...
#	def maximizeWindow(self):
	...
	def bindHover(self, b):
		b.bind('<Any-Enter>', lambda event, btn=b, cmd=const.CMD_ENTER: \
			   self(evt=event, btn=btn, cmd=cmd) )
		b.bind('<Any-Leave>', lambda event, btn=b, cmd=const.CMD_LEAVE: \
			   self(evt=event, btn=btn, cmd=cmd) )
...
#	def createFrame(self)
	...
	def addCtrlButton(self, t, s, c):
		...
		b.bag = {}; b.bag[const.CMD_SORT] = s
		b.configure(command = lambda btn=b, cmd=const.CMD_SORT: \
					self(btn=btn, cmd=cmd) )
		if self.sortMethod == s: b.configure(bg="#%02x%02x%02x" % const.COLOR_CURR_BTN)
...
# нарисовать кнопки, открывающие фильмы
		...
		for m in mlist:
			...
			x += 1
			if x <= (partNo + numb) and x > partNo: pass
			else: continue
	...
	def __call__(self, evt='', btn='', cmd=''):
		...
		if evt:
			if evt.state & const.BIT_CONTROL:
				if not self.ctrlPressed: print 'switch ctrl to 1'
				self.ctrlPressed = True
			else:
				if self.ctrlPressed: print 'switch ctrl to 0'
				self.ctrlPressed = False

		if cmd:
			if cmd == const.CMD_CTRL_DOWN:
				self.ctrlPressed = True; print 'ctrl-btn-evt press'
			elif cmd == const.CMD_CTRL_UP:
				self.ctrlPressed = False; print 'ctrl-btn-evt release'
